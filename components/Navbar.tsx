    ...
    <nav className="fixed top-0 left-0 w-full z-[9999] px-6 md:px-8 lg:px-12 transition-all duration-700">
        ...
        <div className={`hidden lg:flex items-center gap-8 xl:gap-12 font-sans text-xs tracking-[0.2em] uppercase transition-colors duration-500 ${isDarkText ? 'text-sbh-charcoal/80' : 'text-white/90 drop-shadow-md'
          ...
          <div className="flex items-center gap-1 ml-4 border-l border-current/20 pl-4">
            ...
            <button
              ...
              className="hover:text-sbh-blue transition-colors duration-300 font-bold"
            >
      ...
      {menuOpen && (
        <div className="fixed inset-0 bg-sbh-cream z-[999] flex flex-col items-center justify-center gap-8 animate-fade-in overscroll-contain">
          ...
          <button onClick={() => handleNavClick('home')} className={`text-xl font-serif font-light italic ${isActive('home') ? 'text-sbh-blue' : 'text-sbh-charcoal'}`}>{t.nav.home}</button>
          ...
          <button onClick={() => handleNavClick('destinations')} className={`text-xl font-serif font-light italic ${isActive('destinations') ? 'text-sbh-blue' : 'text-sbh-charcoal'}`}>{t.nav.destination}</button>
          <button onClick={() => handleNavClick('services')} className={`text-xl font-serif font-light italic ${isActive('services') ? 'text-sbh-blue' : 'text-sbh-charcoal'}`}>{t.nav.concierge}</button>
          <button onClick={() => handleNavClick('about')} className={`text-xl font-serif font-light italic ${isActive('about') ? 'text-sbh-blue' : 'text-sbh-charcoal'}`}>{t.nav.spirit}</button>
          <button onClick={() => handleNavClick('contact')} className={`text-xs font-sans tracking-[0.3em] uppercase mt-8 border-b pb-1 ${isActive('contact') ? 'text-sbh-blue border-sbh-blue' : 'text-sbh-charcoal border-sbh-charcoal'}`}>{t.nav.contact}</button>

          {/* Language Switcher Mobile */}
          <button
            onClick={() => setLanguage(language === 'fr' ? 'en' : 'fr')}
            className="flex items-center gap-2 mt-8 text-sm font-sans tracking-widest uppercase text-sbh-charcoal/60 hover:text-sbh-blue transition-colors"
          >
            <Globe size={16} />
