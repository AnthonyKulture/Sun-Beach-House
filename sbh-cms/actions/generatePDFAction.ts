...
export const GeneratePDFAction: DocumentActionComponent = (props) => {
    ...
    return {
        ...
        onHandle: async () => {
            ...
            try {
                // Call the Next.js API to generate PDF WITHOUT pricing
                const apiUrl = process.env.SANITY_STUDIO_PREVIEW_URL || 'http://localhost:3000'
                const response = await fetch(
                    `${apiUrl}/api/generate-pdf?villaId=${villaId}&lang=${language}&includePricing=false&t=${Date.now()}`
                )
                ...
                a.href = url
                a.download = `villa-${villaId}-brochure-sans-tarifs.pdf`
                ...
                console.log('PDF sans tarifs généré avec succès')
            } catch (error) {
